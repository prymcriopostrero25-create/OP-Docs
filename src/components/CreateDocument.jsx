    ...
    <div className="create-document-overlay" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="create-document-modal" role="dialog" aria-modal="true" aria-labelledby="create-document-title">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {canChangeStatus && <div className="create-field"><label htmlFor="document-status">Initial status</label><select id="document-status" name="status" value={form.status} onChange={updateField}><option>Draft</option><option>In review</option><option>For signature</option></select></div>}
