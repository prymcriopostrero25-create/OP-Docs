...
export default function ActivityLogPage() {
  ...
  const entries = files.filter(file => `${file.subject || ''} ${file.activity || ''} ${file.type || ''}`.toLowerCase().includes(query.trim().toLowerCase())).sort((a, b) => {
  ...
  return <main className="dashboard-content activity-log-page">
    ...
    <section className="activity-log-panel" aria-labelledby="history-title" aria-busy={loading}>
      <div className="activity-log-heading">
        <div><h2 id="history-title">Upload history <span>{files.length}</span></h2><p>All times in Philippine time (UTC+8)</p></div>
      ...
      {loading ? <p className="activity-log-state" role="status">Loading activity...</p> : <>
        ...
        <footer className="activity-log-footer" role="status">Showing {entries.length} of {files.length} {files.length === 1 ? 'activity' : 'activities'}</footer>
      </>}
    </section>
  </main>
}
