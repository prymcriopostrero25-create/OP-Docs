  ...
  return <main className="dashboard-content admin-page">
    ...
    <form className="settings-layout" onSubmit={save}>
      <div className="settings-main">
        <section className="settings-card"><div className="settings-card-title"><span>01</span><div><h2>Office information</h2><p>Details displayed across official records.</p></div></div><div className="settings-fields"><label>Office name<input value={settings.office} onChange={(e) => update('office', e.target.value)} /></label><label>Official email<input type="email" value={settings.email} onChange={(e) => update('email', e.target.value)} /></label><label>Document reference prefix<input value={settings.prefix} onChange={(e) => update('prefix', e.target.value.toUpperCase())} maxLength="6" /></label></div></section>
